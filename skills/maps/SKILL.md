...
### Large local business prospect lists

For sales-prospecting requests like “find 250 privately owned businesses within 5 miles likely doing $1M+/year,” use direct Overpass collection across broad commercial tags plus custom filtering, scoring, deduping, and balanced CSV export. See `references/local-business-prospect-research.md` for the full workflow, output columns, verification checks, and revenue/ownership caveats.

When the follow-up task is to find owners/managers and outreach emails for the prospect list, enrich the curated CSV/CRM by crawling each business website's homepage/contact/about/team pages, classifying human vs generic emails, deriving clearly-labeled fallback `info@domain` addresses, and verifying any generated CRM HTML. See `references/local-business-contact-enrichment.md` for the enrichment workflow and quality filters.
