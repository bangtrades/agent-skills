---
name: obsidian-cli-guard
description: >
  Correct-syntax guard for the official Obsidian CLI (1.12+), preventing 13
  documented silent failures where the CLI exits 0 with wrong or empty output
  (e.g. `tasks todo` scoping to a nonexistent active file — use `tasks all todo`;
  `tags counts` — use `tags all counts`; `create` without `silent` popping the GUI).
  Use when running any obsidian CLI command against the vault. Trigger on
  "obsidian cli", "vault search", "backlinks", "orphans", "unresolved",
  "vault tags", "vault tasks", "property set", "base query", or any
  index-powered vault operation. Prefer the CLI for index-powered ops; fall
  back to file tools when Obsidian is not running or for simple read/write.
triggers:
  - obsidian cli
  - vault search
  - backlinks
