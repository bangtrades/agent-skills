...
- For structured extraction tools, probe **semantic validity**, not only transport success: define one plausibility/schema check and one authoritative comparison before dispatch. A 200 response containing invented, placeholder-like, or internally inconsistent fields is a failed boundary probe; discard it, log the failure, and route the slice to the authoritative fallback.
...
- **Disjoint frontend files still share the CSS cascade.** When parallel slices load styles into one document, assign and pin a unique selector namespace per slice (for example `world-*`, `store-*`, `chrome-*`) in every prompt; run a cross-slice duplicate-selector scan before visual verification.
...
### Dispatch Discipline (added 2026-07 — PSG build; the single biggest wall-clock lever)

**The board leads the work — never trails it.** Move a tracker issue to **In Progress the moment you commit to working it, BEFORE the agent (or inline build) starts** — not when it finishes, not in a batch at the end. Post a one-line "dispatching / starting" comment at that same moment. Mark Done only on verified artifact. An issue that jumps `created → Done` and never showed In Progress is a process failure: the operator watching the board could not see what was being worked. (PSG evidence: six surface issues went created→Done with `startedAt` null — the operator never saw them light up. Fix: transition-before-work is mandatory.)

**Inline-first for coupled work; fan out only when work is genuinely file-disjoint.** Fan-out pays off when slices own separate files with a thin shared contract. It is a *net loss* when the "slices" all edit a shared design-system file, a shared chart/UI kit, or assemble into one single-file build — coordination and merge cost exceed any parallelism, and every sibling collides on the shared surface. For a static-first single-file app whose surfaces share one token sheet + one render kit, **build inline and sequentially.** (PSG evidence: ~26 min of dispatch thrashing produced one orphan util file and zero usable app code; the inline rebuild produced the entire 7-surface app + chart kit + single-file build in ~11 min. Going inline at the decision point would have cut ~26 min — nearly a fifth — off a 2h23m run.)

**Do not fan out into an attended, actively-messaging session.** Mid-turn user messages interrupt the turn and kill in-flight subagents. If the operator is present and iterating, build inline or negotiate an explicit quiet window before a large fan-out. Detect a dead agent fast — a return with **0 tool uses in <10s is a killed/failed agent, not a fast one** — and do not read its output as signal.

**Two-strike rule.** If a dispatch wave comes back dead or junk twice, switch to inline immediately. A third dispatch attempt is almost always slower than just building it.

**Killed-agent artifacts are UNVERIFIED by default.** A file left behind by an interrupted agent may follow a different code path than the canonical one — at PSG an orphan generator used a different RNG draw order and would have produced a divergent ledger. Never reuse such a file until it is checked against the canonical builder; salvage only pure, side-effect-free utilities, and only after diffing.
...
- Do not fan out agents into a session where the operator is actively sending messages — mid-turn interrupts kill in-flight subagents. Inline, or negotiate a quiet window first.
- Do not "slice" work that shares a single design-system file, render kit, or single-file build target — that is coupled work; build it inline. Reserve fan-out for file-disjoint slices.
- Do not let a tracker issue jump created→Done. Move it to In Progress before the work starts, or the board stops reflecting reality.
- Do not reuse a file left by a killed agent without verifying it against the canonical builder — it may follow a divergent code path.
