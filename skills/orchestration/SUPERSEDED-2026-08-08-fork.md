...
3. **Patch this skill in the registry** (save/overwrite the skill so the update persists across sessions) and append a dated entry to the Changelog below: date, project, what was learned, what changed in the skill. Mirror the update to `~/Cortana/cortana-skill-registry/orchestration/SKILL.md` (hand the operator a copy command if the registry is host-protected from the session).
